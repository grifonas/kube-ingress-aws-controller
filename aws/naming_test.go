...
func TestNormalization(t *testing.T) {
	for _, test := range []struct {
		...
		{"foo", nil, map[string]string{"": "foo-fd80709"}},
		{"foo", []string{"bar"}, map[string]string{"": "foo-1f01f4d"}},
		{"foo", []string{"fake-cert-arn"}, map[string]string{"": "foo-f398861"}},
		{"test/with-invalid+chars", []string{"bar"}, map[string]string{"": "test-with-invalid-chars-1f01f4d"}},
		{"-apparently-valid-name", []string{"bar"}, map[string]string{"": "apparently-valid-name-1f01f4d"}},
		{"apparently-valid-name-", []string{"bar"}, map[string]string{"": "apparently-valid-name-1f01f4d"}},
		{"valid--name", []string{"bar"}, map[string]string{"": "valid-name-1f01f4d"}},
		{"valid-----name", []string{"bar"}, map[string]string{"": "valid-name-1f01f4d"}},
		{"foo bar baz zbr", []string{"bar"}, map[string]string{"": "foo-bar-baz-zbr-1f01f4d"}},
		{"very long cluster where lb but not stack id needs to be truncated", []string{"bar"}, map[string]string{
			"elb":   "id-needs-to-be-truncated-1f01f4d",
			"stack": "very-long-cluster-where-lb-but-not-stack-id-needs-to-be-truncated-1f01f4d",
		}},
		{"-no---need---to---truncate-", nil, map[string]string{"": "no-need-to-truncate-fd80709"}},
		{
